...
widgets.define('drag-source', (el, options = {}) => {
  ...
  //
  // let q
  // const transferableImageSource = (q = el.getAttribute('data-image-source'))
  //   ? draggedContainer.querySelector(q)
  //   : null
  //
  ...
  // const flavor = (el.getAttribute('data-flavor') || '{all}').split(',')
  ...
  //
  // const startDrag = (e) => {
  //   const targetSelector = flavor[0] === '{all}'
  //     ? '[data-drop]'
  //     : flavor.map(f => `[data-drop][data-flavor*='${flavor}']`).join(',')
  //
  ...
  //   potentialTargets = targetContainer.querySelectorAll(targetSelector)
  //
  ...
  //     const dropFlavor = (target.getAttribute('data-flavor') || '{all}').split(',')
  //
  //     const matchedFlavors = dropFlavor[0] === '{all}'
  //       ? flavor
  //       : flavor.filter(f => dropFlavor.indexOf(f) !== -1)
  //
...
widgets.define('drop-target', (el) => {
  ...
  //
  // el.dropHandle = handlerName
  //   ? (transferable, flavors, index, sourceElement) => {
  //     handler.call(el, transferable, flavors, index, sourceElement)
  //   }
  //   : (transferable, flavors, index, sourceElement) => {
  //     console.log('transferable =', transferable, 'with flavors', flavors)
  //   }
})
